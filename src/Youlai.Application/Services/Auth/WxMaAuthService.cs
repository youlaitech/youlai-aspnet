...
internal sealed class WxMaAuthService : IWxMaAuthService
{
    ...
    public async Task<WxMaLoginResultDto> SilentLoginAsync(string code, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new BusinessException(ResultCode.UserLoginException, "微信登录失败：无法获取用户标�?);
        }

        // 2. 查找是否已绑定用�?        var social = await _dbContext.SysUserSocials
            .AsNoTracking()
    ...
    public async Task<AuthenticationTokenDto> PhoneLoginAsync(string loginCode, string phoneCode, CancellationToken cancellationToken = default)
    {
        ...
        // 2. 获取手机�?        var mobile = await GetPhoneNumberAsync(phoneCode, cancellationToken);

        _logger.LogInformation("微信小程序手机号快捷登录：openId={OpenId}, mobile={Mobile}", openId, mobile);

        // 3. 查询或创建用�?        var user = await FindOrCreateUserAsync(mobile, cancellationToken);
    ...
    {
        // 1. 验证短信验证�?        await ValidateSmsCodeAsync(mobile, smsCode);

        // 2. 查询或创建用�?        var user = await FindOrCreateUserAsync(mobile, cancellationToken);
    ...
    private async Task<string> GetAccessTokenAsync(CancellationToken cancellationToken)
    {
        ...
        // 缓存 token（提�?分钟过期�?        var expiresIn = Math.Max(response.ExpiresIn - 300, 60);
    ...
    private async Task BindWechatOpenIdAsync(long userId, string openId, string? unionId, string? sessionKey, CancellationToken cancellationToken)
    {
        ...
        {
            // 绑定失败不影响登�?            _logger.LogWarning(ex, "绑定微信 openid 失败，userId={UserId}, openId={OpenId}", userId, openId);
        }
    }
    ...
    private async Task<AuthenticationTokenDto> GenerateTokenByUserIdAsync(long userId, CancellationToken cancellationToken)
    {
        ...
        if (user is null)
        {
            throw new BusinessException(ResultCode.UserLoginException, "用户不存�?);
        }

        // 查询用户的所有角色及其数据权�?        var rolesQuery =
