...
namespace Youlai.Application.Services.File;
...
internal sealed class FileService : IFileService
{
    ...
    private IFileStorage ResolveStorage()
    {
        ...
        {
            throw new BusinessException(ResultCode.SystemError, "Oss:Type 未配�?);
        }

        var storage = _storages.FirstOrDefault(item => item.Type.Equals(type, StringComparison.OrdinalIgnoreCase));
        if (storage is null)
        {
            throw new BusinessException(ResultCode.SystemError, $"未找到文件存储实�? {type}");
        }
