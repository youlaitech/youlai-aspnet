...
internal sealed class SystemDictService : ISystemDictService
{
    ...
    public SystemDictService(IYoulaiDbContext dbContext, ICurrentUser currentUser, ISseService sseService)
    {
    ...
    public async Task<bool> DeleteDictsAsync(string ids, CancellationToken cancellationToken = default)
    {
        ...
        if (idList.Count == 0)
        {
            throw new BusinessException(ResultCode.InvalidUserInput, "删除的字典数据为�?);
        }
