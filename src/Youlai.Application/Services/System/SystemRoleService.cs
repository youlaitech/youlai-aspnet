...
/// </summary>
/// <remarks>
/// 提供角色维护、角色下拉选项与角色菜单权限分配能�?/// </remarks>
internal sealed class SystemRoleService : ISystemRoleService
{
    ...
    public SystemRoleService(IYoulaiDbContext dbContext, ICurrentUser currentUser, IRolePermsCacheInvalidator rolePermsCacheInvalidator, JwtTokenManager jwtTokenManager)
    {
    ...
    public async Task<bool> SaveRoleAsync(RoleForm formData, CancellationToken cancellationToken = default)
    {
        ...
        if (roleId.HasValue)
        {
            ...
            if (oldRole is null)
            {
                throw new BusinessException(ResultCode.InvalidUserInput, "角色不存�?);
            }
        }
        ...
        // 数据权限发生变化时，失效该角色关联用户的登录态（JWT tokenVersion�?        if (oldDataScope != oldRole.DataScope)
        {
