...
internal sealed class SystemDeptService : ISystemDeptService
{
    ...
    public SystemDeptService(IYoulaiDbContext dbContext, ICurrentUser currentUser, IDataPermissionService dataPermissionService)
    {
    ...
    public async Task<IReadOnlyCollection<Option<long>>> GetDeptOptionsAsync(CancellationToken cancellationToken = default)
    {
        ...
        // 数据权限过滤：下拉选项也需要过滤，避免越权拿到部门�?        q = _dataPermissionService.Apply(q, d => d.Id, d => d.CreateBy ?? 0);
