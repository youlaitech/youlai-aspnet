...
/// </summary>
/// <remarks>
/// 提供角色维护、角色下拉选项与角色菜单权限分配能力/// </remarks>
internal sealed class SystemRoleService : ISystemRoleService
{
    ...
    public SystemRoleService(IDbContext dbContext, ICurrentUser currentUser, IRolePermsCacheInvalidator rolePermsCacheInvalidator, JwtTokenManager jwtTokenManager)
    {
    ...
    {
        var roleIds = IdCollectionExtensions.ParsePositiveLongIds(ids);
        if (roleIds.Count == 0)
        {
            throw new BusinessException(ResultCode.InvalidUserInput, "角色ID不能为空");
        }
        ...
        foreach (var roleId in roleIds)
        {
            ...
            if (hasAssignedUsers)
            {
                throw new BusinessException(ResultCode.InvalidUserInput, $"角色 [{role.Name}] 已分配用户，请先解除关联");
            }
